...
export default defineComponent({
  ...
  watch: { 
    modelValue: function(newVal : number, oldVal: number) { // watch it
      if (newVal != oldVal) {
        this.init()
      }
    }
  ...
  computed: {
    ...
    getState(): string {
      return ("state = " + JSON.stringify(this.state, null, '  ') +
  ...
  methods: {
    init() : void {
      ...
      this.state.maxPosition = this.getHeight(this.getRect("element")) - this.getHeight(this.state.buttonRect);
      this.state.minPosition = 0;
      this.state.currentValue = this.modelValue;
      this.state.scrollPosition = (this.modelValue - this.minValue) / 
    ...
    getScrollLength(): number {
      return this.isVert
        ? this.state.elementRect.height
        : this.state.elementRect.width;
    },
    moveMouse(): void {
      ...
      let newValue = (this.state.scrollPosition - this.state.minPosition)
        / (this.state.maxPosition - this.state.minPosition)
    ...
    mouseMove(event: MouseEvent) {
      if (this.state.startY == -1) {
        ...
        buttonPosition = Math.min(this.state.maxPosition, Math.max(buttonPosition, this.state.minPosition));
